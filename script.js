...
function updateAuthUI() {
    ...
    accountLinks.forEach(function (link) {
        ...
        if (user) {
            ...
            link.setAttribute("href", "user-dashboard.html");
...
/* =====================================================
   ...
   Front-end demonstration for GitHub Pages.
   ===================================================== */

const ADMIN_SESSION_KEY = "techstoreAdminSession";
...
function setupUserDashboardPage() {
    const container = document.getElementById("userDashboardContainer");
    if (!container) return;

    const loggedOut = document.getElementById("dashboardLoggedOut");
    const loggedIn = document.getElementById("dashboardLoggedIn");
    const user = getCurrentUser();
    ...
    if (avatar) avatar.textContent = (user.name || "T").trim().charAt(0).toUpperCase() || "T";

    const orders = typeof getUserOrders === "function" ? getUserOrders(user.id) : getOrders().filter(function (order) {
    ...
    const totalSpent = orders.reduce(function (sum, order) {
        ...
        return sum + (Number(order.finalTotal) || 0);
    }, 0);
    ...
    const spend = document.getElementById("dashboardSpend");
    if (orderCount) orderCount.textContent = orders.length;
    if (wishlistCount) wishlistCount.textContent = wishlist.length;
    if (cartCount) cartCount.textContent = cart.reduce(function (sum, item) { return sum + (Number(item.quantity) || 0); }, 0);
    if (spend) spend.textContent = formatPrice(totalSpent);

    const recent = document.getElementById("dashboardRecentOrders");
    if (recent) {
        const recentOrders = orders.slice(0, 4);
        if (recentOrders.length === 0) {
            recent.innerHTML = '<div class="dashboard-no-data"><span>📦</span><p>No orders yet.</p><a href="products.html" class="btn btn-primary">Start Shopping</a></div>';
        } else {
            recent.innerHTML = recentOrders.map(function (order) {
                const status = String(order.status || "Processing");
                return `
                    <div class="dashboard-order-row">
                        <div><strong>${escapeCheckoutHTML(order.orderId)}</strong><span>${escapeCheckoutHTML(formatOrderDate(order.orderDate))}</span></div>
                        <div><strong>${formatPrice(Number(order.finalTotal) || 0)}</strong><span class="order-status ${getOrderStatusClass(status)}">${escapeCheckoutHTML(status)}</span></div>
                        <a href="order-details.html?id=${encodeURIComponent(order.orderId)}" aria-label="View ${escapeCheckoutHTML(order.orderId)}">View</a>
                    </div>
                `;
            }).join("");
        }
    ...
    const logout = document.getElementById("dashboardLogoutBtn");
...
function renderAdminDashboard() {
    ...
    if (salesTotal) salesTotal.textContent = formatPrice(revenue);

    const ordersTable = document.getElementById("adminOrdersTable");
    if (ordersTable) {
        const sortedOrders = orders.slice().sort(function (a, b) {
            return new Date(b.orderDate || 0) - new Date(a.orderDate || 0);
        }).slice(0, 12);

        if (sortedOrders.length === 0) {
            ordersTable.innerHTML = '<tr><td colspan="5" class="admin-no-data">No orders available.</td></tr>';
        } else {
            ordersTable.innerHTML = sortedOrders.map(function (order) {
                const customer = order.customer || {};
                const status = String(order.status || "Processing");
                return `
                    <tr>
                        <td><strong>${escapeCheckoutHTML(order.orderId)}</strong><small>${escapeCheckoutHTML(formatOrderDate(order.orderDate))}</small></td>
                        <td>${escapeCheckoutHTML(customer.name || "Customer")}<small>${escapeCheckoutHTML(customer.email || "")}</small></td>
                        <td><strong>${formatPrice(Number(order.finalTotal) || 0)}</strong></td>
                        <td><span class="order-status ${getOrderStatusClass(status)}">${escapeCheckoutHTML(status)}</span></td>
                        <td>
                            <select class="admin-status-select" data-order-id="${escapeCheckoutHTML(order.orderId)}" aria-label="Change status for ${escapeCheckoutHTML(order.orderId)}">
                                ${["Processing", "Shipped", "Delivered", "Cancelled"].map(function (option) {
        ...
        ordersTable.querySelectorAll(".admin-status-select").forEach(function (select) {
            select.addEventListener("change", function () {
                const targetId = this.dataset.orderId;
                const newStatus = this.value;
                const allOrders = getOrders();
                const target = allOrders.find(function (order) { return String(order.orderId) === String(targetId); });
                if (!target) return;
                target.status = newStatus;
                if (newStatus === "Cancelled" && !target.cancelledDate) target.cancelledDate = new Date().toISOString();
                if (newStatus !== "Cancelled") delete target.cancelledDate;
    ...
    if (usersTable) {
        if (users.length === 0) {
            usersTable.innerHTML = '<tr><td colspan="3" class="admin-no-data">No registered users yet.</td></tr>';
        } else {
            usersTable.innerHTML = users.slice().sort(function (a, b) { return Number(a.id) - Number(b.id); }).map(function (user) {
                return `<tr><td><strong>${escapeCheckoutHTML(user.name || "—")}</strong></td><td>${escapeCheckoutHTML(user.email || "—")}</td><td>${escapeCheckoutHTML(String(user.id || "—"))}</td></tr>`;
            }).join("");
        }
    }
        ...
        productGrid.innerHTML = products.map(function (product) {
            return `
                <article class="admin-product-card">
                    ...
                    <div><span>${escapeCheckoutHTML(product.category || "")}</span><h3>${escapeCheckoutHTML(product.name || "Product")}</h3><strong>${formatPrice(Number(product.price) || 0)}</strong></div>
                    <a href="product-details.html?id=${encodeURIComponent(product.id)}">View</a>
                </article>
            `;
        }).join("");
    }
...
function setupAdminPage() {
    ...
    if (getAdminSession()) showAdminDashboard();
    else showAdminLogin();
    ...
    if (loginForm && !loginForm.dataset.bound) {
        ...
        loginForm.addEventListener("submit", function (event) {
            ...
            if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
                ...
                errorBox.textContent = "Invalid administrator credentials.";
                errorBox.classList.add("show");
            }
        });
    }
    ...
    /* Keep the admin view current when orders/users change in another tab
       of the same browser, or when the administrator returns to the tab. */
    if (!window.__techStoreAdminRefreshBound) {
        window.__techStoreAdminRefreshBound = true;
        window.addEventListener("storage", function (event) {
            if ([ORDERS_KEY, USERS_KEY].indexOf(event.key) !== -1 && getAdminSession()) {
                renderAdminDashboard();
            }
        });
        window.addEventListener("focus", function () {
            if (getAdminSession() && document.getElementById("adminDashboardView")?.style.display !== "none") {
                renderAdminDashboard();
            }
        });
    }
